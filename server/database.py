...
def init_db():
    ...
    try:
        ...
        if not hasattr(root, "menus"):
            ...
            root.menus["Tomato Spaghetti"] = MainDish("Tomato Spaghetti", 10, "Spaghetti with tomato sauce", "Pasta", 5, ["spaghetti", "tomato sauce", "garlic", "basil"], photo="https://food.fnr.sndimg.com/content/dam/images/food/fullset/2021/02/05/Baked-Feta-Pasta-4_s4x3.jpg.rend.hgtvcom.1280.1280.suffix/1615916524567.jpeg")
            ...
            root.menus["Chocolate Lava Cake"] = Dessert("Chocolate Lava Cake", 8, "Warm chocolate cake with a gooey molten center", "Dessert", 4, ["chocolate", "butter", "sugar", "eggs", "flour"], photo="https://preppykitchen.com/wp-content/uploads/2022/03/Chocolate-Lava-Cake-Recipe.jpg")
        ...
        if not hasattr(root, "stats"):
            ...
            root.stats["2024-02-04"] = Stat(date="2024-02-04", cost=60, income=300)
            root.stats["2024-02-05"] = Stat(date="2024-02-05", cost=40, income=200)
            root.stats["2024-02-06"] = Stat(date="2024-02-06", cost=55, income=250)
            # March
            ...
            root.stats["2024-03-04"] = Stat(date="2024-03-04", cost=80, income=600)
            root.stats["2024-03-05"] = Stat(date="2024-03-05", cost=45, income=400)
            root.stats["2024-03-06"] = Stat(date="2024-03-06", cost=65, income=550)
            # April
            ...
            root.stats['2024-04-04'] = Stat(date='2024-04-04', cost=90, income=150)
            root.stats['2024-04-05'] = Stat(date='2024-04-05', cost=55, income=180)
            root.stats['2024-04-06'] = Stat(date='2024-04-06', cost=75, income=220)
            
        if not hasattr(root, "popular"):
            root.popular = BTrees.OOBTree.BTree()
            root.popular["Tomato Spaghetti"] = Popular(name="Tomato Spaghetti", point=3)
            root.popular["Burger"] = Popular(name="Burger", point=3)
            root.popular["Coke"] = Popular(name="Coke", point=11)
            root.popular["Ice Cream"] = Popular(name="Ice Cream", point=4)
            root.popular["Sushi Platter"] = Popular(name="Sushi Platter", point=7)
            root.popular["Ramen"] = Popular(name="Ramen", point=10)
            root.popular["Grilled Salmon"] = Popular(name="Grilled Salmon", point=5)
            root.popular["Chocolate Lava Cake"] = Popular(name="Chocolate Lava Cake", point=1)
            root.popular["Mango Sticky Rice"] = Popular(name="Mango Sticky Rice", point=7)
            root.popular["Iced Matcha Latte"] = Popular(name="Iced Matcha Latte", point=3)
